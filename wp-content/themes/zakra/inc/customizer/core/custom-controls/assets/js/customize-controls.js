	...
	function ( $ ) {
		...
		wp.customize.controlConstructor['zakra-background'] = wp.customize.Control.extend( {
			...
			ready : function () {
				...
				control.initZakraBackgroundControl();

			},

			initZakraBackgroundControl : function () {

				var control     = this,
				    value       = control.setting._value,
				    colorpicker = control.container.find( '.zakra-color-picker-alpha' );
				...
				control.container.on( 'click', '.background-image-upload-remove-button', function ( e ) {
					...
					if ( preview.length ) {
						preview.removeClass().addClass( 'placeholder' ).html( ZakraCustomizerControlBackground.placeholder );
					}
 ...
 * @package Zakra
 */
wp.customize.controlConstructor['zakra-dimensions'] = wp.customize.Control.extend( {
 ...
 * @package Zakra
 */
wp.customize.controlConstructor[ 'zakra-dropdown-categories' ] = wp.customize.Control.extend( {
		...
		wp.customize.controlConstructor['zakra-fontawesome'] = wp.customize.Control.extend(
			{
				ready: function () {
					...
					control.initZakraFontawesomeControl();
				},

				initZakraFontawesomeControl: function() {
					var control       = this,
						selector      = control.selector,
						elSelector    = $( selector ).find( 'select' ),
						faData        = [],
						value         = control.setting._value,
						data          = window['ZakraCustomizerControlFontawesome' + this.id],
						faDataCounter = 0,
						faSelect;
...
(
	function ( $ ) {
		...
		wp.customize.controlConstructor['zakra-group'] = wp.customize.Control.extend( {
			...
			ready : function () {
				...
				this.container.on( 'zakra_settings_changed', control.onOptionChange );
			...
			registerToggleEvents : function () {
				...
				$( '.wp-full-overlay-sidebar-content' ).click(
					function ( e ) {
						if ( ! $( e.target ).closest( '.zakra-field-settings-modal' ).length ) {
							$( '.zakra-group-toggle-icon.open' ).trigger( 'click' );
						}
					}
				);
				...
				control.container.on(
					'click',
					'.zakra-group-wrap .zakra-group-toggle-icon',
					function ( e ) {
						...
						var $this          = $( this ),
						    parent_wrap    = $this.closest( '.customize-control-zakra-group' ),
						    is_loaded      = parent_wrap.find( '.zakra-field-settings-modal' ).data( 'loaded' ),
						    parent_section = parent_wrap.parents( '.control-section' );

						if ( $this.hasClass( 'open' ) ) {
							parent_wrap.find( '.zakra-field-settings-modal' ).hide();
						} else {

							/* Close popup when another popup is clicked */
							var get_open_popup = parent_section.find( '.zakra-group-toggle-icon.open' );
							if ( get_open_popup.length > 0 ) {
								get_open_popup.trigger( 'click' );
							}

							if ( is_loaded ) {
								parent_wrap.find( '.zakra-field-settings-modal' ).show();
							} else {

								var fields     = control.params.zakra_fields,
								    modal_wrap = $( ZakraCustomizerControlGroup.group_modal_tmpl ),
								    device     = $( '#customize-footer-actions .active' ).attr( 'data-device' );

								parent_wrap.find( '.zakra-field-settings-wrap' ).append( modal_wrap );
								parent_wrap.find( '.zakra-fields-wrap' ).attr( 'data-control', control.params.name );
								control.zakra_render_field( parent_wrap, fields, control );
								parent_wrap.find( '.zakra-field-settings-modal' ).show();
				...
				control.container.on(
					'click',
					'.zakra-group-wrap > .customizer-text',
					function ( e ) {

						e.preventDefault();
						e.stopPropagation();

						$( this ).find( '.zakra-group-toggle-icon' ).trigger( 'click' );
			...
			zakra_render_field : function ( wrap, fields, control_element ) {

				var control             = this;
				var zakra_field_wrap = wrap.find( '.zakra-fields-wrap' );
				var fields_html         = '';
				var control_types       = [];
				...
				if ( 'undefined' != typeof fields.tabs ) {
					...
					var counter = 0;
					fields_html += '<div id="' + control_element.params.name + '-tabs" class="zakra-group-tabs">';

					fields_html += '<ul class="zakra-group-list">';
					_.each(
						fields.tabs,
						function ( value, key ) {
							var li_class = '';
					...
					fields_html += '<div class="zakra-tab-content" >';
					_.each(
						fields.tabs,
					...
					zakra_field_wrap.html( fields_html );
				...
				_.each(
					...
					function ( control_type, index ) {
						...
						switch ( control_type.key ) {
							...
							case 'zakra-color':
								control.initColorControl( zakra_field_wrap, control_element, control_type.name );
								break;

							case 'zakra-background':
								control.initBackgroundControl( control_element, control_type, control_type.name );
								break;

							case 'zakra-typography':
				...
				wrap.find( '.zakra-field-settings-modal' ).data( 'loaded', true );
			...
			generateFieldHtml : function ( fields_data, field_values ) {
				...
				_.each(
					...
					function ( attr, index ) {
						...
						if ( (
							     'zakra-typography' === attr.control
						     ) && controlsType.includes( attr.control ) ) {
							attr.languages = ZakraCustomizerControlTypographySubsets;
						}
			...
			initColorControl : function ( wrap, control_elem, name ) {
				...
				var control     = this;
				var colorpicker = wrap.find( '.customize-control-zakra-color .zakra-color-picker-alpha' );
			...
			getTypographyFonts : function () {
				...
				if ( ! _.isUndefined( ZakraCustomizerControlTypography ) ) {
					return ZakraCustomizerControlTypography;
				}
 ...
 * @package Zakra
 */
wp.customize.controlConstructor[ 'zakra-radio-image' ] = wp.customize.Control.extend( {
	...
	ready : function () {
		...
		this.container.find( 'input[type=range]' ).on( 'input change', function () {
			var value        = jQuery( this ).val(),
				input_number = jQuery( this ).closest( '.slider-wrapper' ).find( '.zakra-range-value .value' );
		...
		this.container.find( '.zakra-slider-reset' ).click( function () {
			var wrapper       = jQuery( this ).closest( '.slider-wrapper' ),
			    input_range   = wrapper.find( 'input[type=range]' ),
			    input_number  = wrapper.find( '.zakra-range-value .value' ),
 ...
 * @package Zakra
 */
wp.customize.controlConstructor['zakra-toggle'] = wp.customize.Control.extend( {
