		...
		wp.customize.bind(
			...
			function () {
				...
				wp.customize.section.each(
					function ( section ) {
						if (
							'zakra_section' !== section.params.type ||
		...
		wp.customize.Panel = wp.customize.Panel.extend(
			{
				...
				isContextuallyActive : function () {
					...
					if ( 'zakra_panel' !== this.params.type ) {
		...
		wp.customize.Section = wp.customize.Section.extend(
			{
				attachEvents         : function () {
					...
					if (
						'zakra_section' !== this.params.type ||
				...
				isContextuallyActive : function () {
					...
					if ( 'zakra_section' !== this.params.type ) {
				...
				wp.customize.control.each(
					function ( ctrl, i ) {
						...
						if ( description.length ) {
							...
							li_wrapper.append( '<i class="zakra-control-tooltip dashicons dashicons-editor-help" title="' + tooltip + '"></i>' );
						}
					}
				);
			}
 ...
 *
 * Currently used for the Zakra Upseel section only.
 */
(
	function ( $, api ) {
		api.sectionConstructor[ 'zakra-upsell-section' ] = api.Section.extend(
			{
